...
`
	ir := compileSPMDSource(t, src)
	// In a go-for loop iterating []float64, the loop index i is
	// Varying[int] sized to the float64 iteration width (e.g., 2 lanes
	// on WASM SIMD128 because float64=8 bytes/lane and the register is
	// 16 bytes). The Varying[int] accumulator alloca must be sized to
	// match — alloca <2 x i32>, NOT int's register-natural <4 x i32>.
	// Without v3's SPMDLaneCount annotation, TinyGo would size it to
	// int's natural width (4 on WASM128) and the load-into-iter-store
	// chain would have mismatched lane counts.
	mustContain(t, ir, "alloca <2 x i32>")
	// Load/store widths must match the alloca width. A 16-byte load from
	// an 8-byte alloca is an out-of-bounds stack access. These assertions
	// catch the v3 follow-up bug where the alloca was correctly 2-wide but
	// createSPMDLoad / UnOp{MUL} still used the 4-wide getLLVMType result.
...
// TestSPMDVaryingLocalMaskedInTail is a regression smoke check that the
// IR for a varying-local accumulator inside a partial-mask go-for still
// contains masked memory ops. 5 iterations on 2-wide SIMD (float64 on
// WASM128) => 2 main iters + 1 tail iter (1 active lane of 2). The
// IR must contain at least one masked.store or <i1>-select — without
// the v3 lift guard preserving the acc alloca, partial-mask machinery
// would not emit any of those for the accumulator path. (The match is
// intentionally loose: this test guards against wholesale loss of
// masked stores; the precise alloca lane width is asserted separately
// in TestSPMDVaryingAllocaLLVMType.)
