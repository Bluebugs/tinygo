...
// TestSPMDVaryingAllocaLLVMType verifies that a Varying[int] accumulator declared
// in an outer (non-SPMD) function and used inside a go for loop whose effective lane
// count differs from the element-natural width is allocated and loaded at the loop's
// actual lane count, not the element-natural default.
//
// Fixture: data is []float64, so on WASM128 the loop runs 2 float64 lanes. The
// accumulator acc is Varying[int] (naturally 4 lanes on WASM128). After the
// spmdPropagateBlockLaneCount pass annotates the entry block with LaneCount=2 and
// the createSPMDLoad fix overrides resultType to match, every load/store of acc
// must use <2 x i32> (2-lane int), not <4 x i32> (4-lane int).
...
`
	ir := compileSPMDSource(t, src)
	// Both the in-loop SPMDLoad and the post-loop reduce.Add read must use 2-lane width.
	mustContain(t, ir, "load <2 x i32>")
	mustNotContain(t, ir, "load <4 x i32>")
	// The reduce intrinsic must operate on 2 lanes, not 4.
	mustContain(t, ir, "llvm.vector.reduce.add.v2i32")
	mustNotContain(t, ir, "llvm.vector.reduce.add.v4i32")
}

// TestSPMDVaryingLocalMaskedInTail verifies that when a Varying[int] accumulator
// is used in a float64-driven loop (2 lanes on WASM128), the tail-phase masked store
// also uses <2 x i32> width. This catches the cascade bug where a <4 x i32> load
// feeds a <4 x i32> add whose result is stored via a masked.store.v4i32 — even though
// the loop mask is <2 x i1>.
//
// The SPMD store merging pass may elide the masked.store in favour of a pack+i64
// store; the absence of masked.store.v4i32 covers both outcomes.
...
`
	ir := compileSPMDSource(t, src)
	// No 4-lane operations must appear in either main body or tail.
	mustNotContain(t, ir, "masked.store.v4i32")
	mustNotContain(t, ir, "masked.load.v4i32")
	// The add must be 2-lane.
	mustNotContain(t, ir, "add <4 x i32>")
}

// TestSPMDSpecializedVariantEmitted is a placeholder for the SPMD function
// specialization emission test (v4 Task 11 / deferred from Task 10).
// It verifies that when a Varying-parameter function is called from a 2-lane
// SPMD context, the call site dispatches to the ".spmd2" variant rather than
// the original function. Skipped until variant emission is implemented.
func TestSPMDSpecializedVariantEmitted(t *testing.T) {
	t.Skip("deferred to v4.1: SPMD callee specialization variant emission not yet implemented")
}
