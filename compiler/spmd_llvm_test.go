...
// TestSPMDSwitchChainTypes verifies that the new switch chain struct types can be
// constructed and their fields accessed correctly.
func TestSPMDSwitchChainTypes(t *testing.T) {
	c := newTestCompilerContext(t)
	defer c.dispose()

	// Create a spmdSwitchCase.
	caseInfo := spmdSwitchCase{
		ifBlock:   5,
		bodyBlock: 10,
		caseMask:  llvm.ConstInt(c.ctx.Int1Type(), 1, false),
	}
	if caseInfo.ifBlock != 5 {
		t.Errorf("caseInfo.ifBlock = %d, want 5", caseInfo.ifBlock)
	}
	if caseInfo.bodyBlock != 10 {
		t.Errorf("caseInfo.bodyBlock = %d, want 10", caseInfo.bodyBlock)
	}
	if caseInfo.caseMask.IsNil() {
		t.Error("caseInfo.caseMask is nil")
	}

	// Create a spmdSwitchChain.
	chain := spmdSwitchChain{
		cases:       []spmdSwitchCase{caseInfo},
		defaultBody: 15,
		doneBlock:   20,
		tagValue:    nil,
	}
	if len(chain.cases) != 1 {
		t.Errorf("len(chain.cases) = %d, want 1", len(chain.cases))
	}
	if chain.defaultBody != 15 {
		t.Errorf("chain.defaultBody = %d, want 15", chain.defaultBody)
	}
	if chain.doneBlock != 20 {
		t.Errorf("chain.doneBlock = %d, want 20", chain.doneBlock)
	}
	if chain.tagValue != nil {
		t.Error("chain.tagValue should be nil")
	}
}

// TestSPMDSwitchIfBlockMapping verifies that the spmdSwitchIfBlocks map can be
// populated and queried correctly.
func TestSPMDSwitchIfBlockMapping(t *testing.T) {
	// Create a map of ifBlock indices to chain indices.
	switchIfBlocks := make(map[int]int)
	...
	// Verify lookups.
	if chainIdx, ok := switchIfBlocks[5]; !ok || chainIdx != 0 {
		t.Errorf("switchIfBlocks[5] = %d, %v, want 0, true", chainIdx, ok)
	}
	if chainIdx, ok := switchIfBlocks[10]; !ok || chainIdx != 0 {
		t.Errorf("switchIfBlocks[10] = %d, %v, want 0, true", chainIdx, ok)
	}
	if chainIdx, ok := switchIfBlocks[15]; !ok || chainIdx != 1 {
		t.Errorf("switchIfBlocks[15] = %d, %v, want 1, true", chainIdx, ok)
	}

	// Verify missing key.
	if _, ok := switchIfBlocks[99]; ok {
		t.Error("switchIfBlocks[99] should not exist")
	}
}

// TestSPMDSwitchChainFields verifies that spmdSwitchChain with multiple cases,
// default body, and done block works correctly.
func TestSPMDSwitchChainFields(t *testing.T) {
	c := newTestCompilerContext(t)
	defer c.dispose()

	// Create multiple cases.
	case1 := spmdSwitchCase{
		ifBlock:   1,
		bodyBlock: 2,
		caseMask:  llvm.ConstInt(c.ctx.Int1Type(), 1, false),
	}
	case2 := spmdSwitchCase{
		ifBlock:   3,
		bodyBlock: 4,
		caseMask:  llvm.ConstInt(c.ctx.Int1Type(), 0, false),
	}
	case3 := spmdSwitchCase{
		ifBlock:   5,
		bodyBlock: 6,
		caseMask:  llvm.ConstInt(c.ctx.Int1Type(), 1, false),
	}

	// Create a chain with 3 cases, default body, and done block.
	chain := spmdSwitchChain{
		cases:       []spmdSwitchCase{case1, case2, case3},
		defaultBody: 7,
		doneBlock:   8,
		tagValue:    nil,
	}
	...
	// Verify fields.
	if len(chain.cases) != 3 {
		t.Errorf("len(chain.cases) = %d, want 3", len(chain.cases))
	}
	if chain.cases[0].ifBlock != 1 {
		t.Errorf("chain.cases[0].ifBlock = %d, want 1", chain.cases[0].ifBlock)
	}
	if chain.cases[1].bodyBlock != 4 {
		t.Errorf("chain.cases[1].bodyBlock = %d, want 4", chain.cases[1].bodyBlock)
	}
	if chain.cases[2].ifBlock != 5 {
		t.Errorf("chain.cases[2].ifBlock = %d, want 5", chain.cases[2].ifBlock)
	}
	if chain.defaultBody != 7 {
		t.Errorf("chain.defaultBody = %d, want 7", chain.defaultBody)
	}
	if chain.doneBlock != 8 {
